...
# os.environ["CUDA_VISIBLE_DEVICES"] = "-1" # uncomment to stop using GPU and use CPU
import cv2
import numpy as np
import sys
import pickle
...
class FasterRCNN:
    @staticmethod
    def test_frcnn(config_file_path: str, trained_model_path: str, input_images_dir_path: str,
                   output_images_dir_path: str):

        with open(config_file_path, 'rb') as f_in:
            C = pickle.load(f_in)

        if not os.path.exists(output_images_dir_path):
            os.makedirs(output_images_dir_path)

        C.model_path = trained_model_path
        ...
        for idx, img_name in enumerate(sorted(os.listdir(input_images_dir_path))):
            if not img_name.lower().endswith(('.bmp', '.jpeg', '.jpg', '.png', '.tif', '.tiff')):
                continue
            print(img_name)
            st = time.time()
            filepath = os.path.join(input_images_dir_path, img_name)
            ...
            cv2.imwrite(os.path.join(output_images_dir_path, img_name), img)
